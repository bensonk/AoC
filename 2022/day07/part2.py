...
def main(args):
  ...
  disk_used = max(sizes.values())
  available_space = 70000000 - disk_used
  target = 30000000 - available_space
  print(f'target size: {target}')

  candidates = [size for path, size in sizes.items() if size >= target ]
  candidates.sort()
  print('Candidates:')
  pprint(candidates)
  print(f'best candidate: {candidates[0]}')
